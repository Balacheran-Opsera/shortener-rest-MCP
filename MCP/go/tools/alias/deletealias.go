...
func DeletealiasHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletealiasTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_aliases",
		mcp.WithDescription("Delete alias"),
		mcp.WithString("domainName", mcp.Description("domain which alias belongs to (string without `http/https` or `/`)")),
		mcp.WithString("aliasName", mcp.Required(), mcp.Description("alias (without `/` at the beginning)")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletealiasHandler(cfg),
	}
}
